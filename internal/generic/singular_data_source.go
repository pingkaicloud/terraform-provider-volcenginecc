...
func (sd *genericSingularDataSource) Read(ctx context.Context, request datasource.ReadRequest, response *datasource.ReadResponse) {
	...
	err = sd.setId(ctx, util.ToString(description.ResourceDescription.Identifier), &response.State)
...
func (sd *genericSingularDataSource) describe(ctx context.Context, conn *cloudcontrol.CloudControl, id string) (*cloudcontrol.GetResourceOutput, error) {
	return tfcloudcontrol.FindResourceByTypeNameAndID(ctx, conn, sd.provider.Region(ctx), sd.ccTypeName, id)
}
...
func (sd *genericSingularDataSource) bootstrapContext(ctx context.Context) context.Context {
	ctx = tflog.SetField(ctx, LoggingKeyCCType, sd.ccTypeName)
